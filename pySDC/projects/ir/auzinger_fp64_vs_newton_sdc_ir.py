...
def benchmark_configuration(
    ...
    for _ in range(repeats):
        ...
        elif method in ('direct-ir', 'newton-sdc-ir'):
            description = make_newton_sdc_ir_description(
                dt=dt,
                num_nodes=num_nodes,
                outer_tol=tol,
...
def create_time_error_plot(output_path, benchmarks, t0, t_end):
    ...
    ax.set_title(f'Auzinger Newton-SDC fp64 vs fp64/32 on [{t0:g}, {t_end:.6g}]')
...
def print_results(benchmarks, t0, t_end, steps, num_nodes, target_tol, maxiter, inner_solver, inner_qi):
    print('Auzinger benchmark: Newton-SDC fp64 vs Newton-SDC IR fp64/32')
    print(
        f't0={t0}, t_end={t_end}, steps={steps}, num_nodes={num_nodes}, '
        f'target_tol={target_tol}, maxiter={maxiter}, inner_solver={inner_solver}, inner_qi={inner_qi}'
    )
    ...
    for entry in benchmarks:
        print(
            ...
            f"{entry['memory_bytes'] / 1024.0:>8.3f}   "
            f"{entry['memory_peak_bytes'] / 1024.0:>9.3f}   "
...
def main(
    ...
    output_path = images_dir / f'{output_stem}_time_error.png'
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compare full-fp64 Newton-SDC against mixed-precision Newton-SDC IR on Auzinger.'
    )
